...
const playlists: PlaylistMeta[] = [
  {
    id: "pl-raids-01",
    title: "\u0420\u0435\u0439\u0434\u0438 \u0434\u043b\u044f \u043d\u043e\u0432\u0430\u0447\u043a\u0456\u0432",
    author: "Nebori Team",
    updated: "\u041e\u043d\u043e\u0432\u043b\u0435\u043d\u043e 2 \u0434\u043d\u0456 \u0442\u043e\u043c\u0443",
    totalVideos: 24,
    duration: "3 \u0433\u043e\u0434\u0438\u043d\u0438 15 \u0445\u0432\u0438\u043b\u0438\u043d",
  ...
  },
  {
    ...
    title: "\u041c\u0435\u0442\u0430-\u0441\u0435\u0437\u043e\u043d #12",
    author: "IronBand",
    updated: "\u041e\u043d\u043e\u0432\u043b\u0435\u043d\u043e 5 \u0434\u043d\u0456\u0432 \u0442\u043e\u043c\u0443",
    totalVideos: 18,
    duration: "2 \u0433\u043e\u0434\u0438\u043d\u0438 48 \u0445\u0432\u0438\u043b\u0438\u043d",
...
function PlayIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 16 16" className="h-4 w-4">
      <path d="M5 3.3v9.4c0 .4.4.7.8.5l7-4.7c.4-.3.4-.8 0-1.1l-7-4.7c-.4-.2-.8 0-.8.6z" fill="currentColor" />
    </svg>
  );
}
...
function ShareIcon() {
  return (
    <svg aria-hidden="true" viewBox="0 0 16 16" className="h-4 w-4">
      <circle cx="12.2" cy="3.3" r="1.6" fill="none" stroke="currentColor" strokeWidth="1.4" />
      <circle cx="3.8" cy="8" r="1.6" fill="none" stroke="currentColor" strokeWidth="1.4" />
      <circle cx="12.2" cy="12.7" r="1.6" fill="none" stroke="currentColor" strokeWidth="1.4" />
      <path d="M5.2 8.7 10.7 12M10.7 4 5.2 7.3" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  );
}
...
export default async function PlaylistPage({ params }: PageProps) {
  ...
  const playlistAuthorHandle = playlist.author.toLowerCase().replace(/\s+/g, "_");
  const playlistAuthorAvatar = `https://api.dicebear.com/9.x/thumbs/svg?seed=${encodeURIComponent(playlist.author)}`;
  const queue = [...videos, ...videos].slice(0, playlist.totalVideos).map((video, index) => ({
    ...video,
    order: index + 1
  }));
    ...
    <section className="-mt-6 mx-auto w-full max-w-[1050px] space-y-4">
      <article className="relative overflow-hidden rounded-b-[6px] border border-[rgba(255,255,255,0.1)] border-t-0 bg-nebori-panel">
        <div className="absolute inset-0">
          <img src={`https://picsum.photos/seed/playlist-cover-${playlist.id}/1600/520`} alt={playlist.title} className="h-full w-full object-cover opacity-20" />
        ...
        <div className="relative grid grid-cols-1 gap-3 p-3 sm:grid-cols-[108px_minmax(0,1fr)_auto] sm:items-end sm:p-4">
          <div className="relative h-[108px] w-[108px] overflow-hidden rounded-[4px] border border-[rgba(255,255,255,0.18)]">
            <img src={`https://picsum.photos/seed/playlist-avatar-${playlist.id}/240/240`} alt={playlist.title} className="h-full w-full object-cover" />
          ...
          <div className="min-w-0">
            <h1 className="text-2xl font-bold leading-tight text-white sm:text-[32px]">{playlist.title}</h1>
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-[#c7cfdf]">
              <ProfileHoverCard handle={playlistAuthorHandle} name={playlist.author} avatar={playlistAuthorAvatar} className="inline-block">
                <img src={playlistAuthorAvatar} alt={playlist.author} className="h-5 w-5 rounded-[2px] border border-[rgba(255,255,255,0.16)] object-cover" />
              </ProfileHoverCard>
              <ProfileHoverCard handle={playlistAuthorHandle} name={playlist.author} avatar={playlistAuthorAvatar} className="inline-block">
                <Link href={`/profile/${playlistAuthorHandle}`} className="font-semibold text-[#dce4f3] hover:text-nebori-accent">
                  {playlist.author}
                </Link>
              </ProfileHoverCard>
              <span>{"\u2022"}</span>
          ...
          <div className="flex items-center gap-2 sm:pb-1">
            ...
            <button className="btn-ghost rounded-[4px] p-2" type="button" aria-label="\u041f\u0435\u0440\u0435\u043c\u0456\u0448\u0430\u0442\u0438">
              <ShuffleIcon />
            </button>
            <button className="btn-ghost rounded-[4px] p-2" type="button" aria-label="\u041f\u043e\u0434\u0456\u043b\u0438\u0442\u0438\u0441\u044f">
              <ShareIcon />
            </button>
          </div>
        </div>
        ...
        <div className="min-w-0">
          <div className="mb-2 flex items-center justify-between border-b border-[rgba(255,255,255,0.08)] pb-2 text-sm">
            <p className="text-nebori-muted">
              {"\u0421\u043e\u0440\u0442\u0443\u0432\u0430\u043d\u043d\u044f"}: <span className="font-semibold text-nebori-text">{"\u0420\u0443\u0447\u043d\u0435"}</span>
            </p>
          ...
          <div className="divide-y divide-[rgba(255,255,255,0.08)]">
            {queue.map((item) => (
              ...
              >
                <span className="pt-7 text-center text-sm font-medium text-[#8d98ae] sm:pt-8 sm:text-base">{item.order}</span>
                <Link href={`/video/${item.id}`} className="group relative block overflow-hidden rounded-[4px] border border-[rgba(255,255,255,0.12)]">
                  <img
                    ...
                    loading="lazy"
                  />
                  <span className="absolute bottom-1 right-1 rounded-[3px] bg-black/80 px-1.5 py-0.5 text-[11px] font-semibold text-white">{item.duration}</span>
                </Link>
                <div className="min-w-0 pt-0.5">
                  <Link href={`/video/${item.id}`} className="line-clamp-2 text-[15px] font-semibold leading-5 text-[#e6e9f3] hover:text-nebori-accent">
                    {item.title}
                  </Link>
                  <ProfileHoverCard
        ...
        <aside className="min-w-0">
          <article className="rounded-[6px] border border-[rgba(255,255,255,0.1)] bg-nebori-panel p-3 lg:sticky lg:top-[84px]">
            <div className="flex items-start justify-between gap-2">
              <h2 className="text-[20px] font-semibold leading-none text-[#edf2ff]">{"\u041f\u0440\u043e \u043f\u043b\u0435\u0439\u043b\u0438\u0441\u0442"}</h2>
              <button
                type="button"
            ...
            <p className="mt-3 text-[13px] leading-6 text-[#c4ccdc]">{playlist.description}</p>
            ...
            <div className="mt-4 space-y-2 border-t border-[rgba(255,255,255,0.08)] pt-3 text-sm">
              <div className="flex items-center justify-between text-nebori-muted">
                <span>{"\u0417\u0430\u0433\u0430\u043b\u043e\u043c \u0432\u0456\u0434\u0435\u043e"}</span>
                <span className="font-semibold text-nebori-text">{playlist.totalVideos}</span>
              </div>
              <div className="flex items-center justify-between text-nebori-muted">
                <span>{"\u0422\u0440\u0438\u0432\u0430\u043b\u0456\u0441\u0442\u044c"}</span>
                <span className="font-semibold text-nebori-text">{playlist.duration}</span>
              </div>
              <div className="flex items-center justify-between text-nebori-muted">
                <span>{"\u0421\u0442\u0432\u043e\u0440\u0435\u043d\u043e"}</span>
                <span className="font-semibold text-nebori-text">{playlist.created}</span>
              </div>
            </div>
          </article>
        </aside>
