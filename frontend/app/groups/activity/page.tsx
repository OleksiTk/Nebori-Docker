...
export default function GroupsActivityPage() {
  const modes = ["Моя стрічка", "Нові пости", "Популярне", "Оголошення"];
    ...
    <section className="mx-auto w-full max-w-[900px] space-y-3">
      <div className="mx-auto w-full max-w-[760px] space-y-3">
        <nav className="flex flex-wrap gap-2">
          {modes.map((mode, idx) => (
            <button key={mode} className={`rounded-sm border px-3 py-1.5 text-sm ${idx === 0 ? "btn-primary" : "btn-ghost"}`} type="button">
              {mode}
            </button>
          ))}
        </nav>
        ...
        <div className="space-y-2.5">
          <article className="rounded-[6px] border border-[rgba(255,255,255,0.1)] bg-nebori-panel p-2.5">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <input
                  type="text"
                  placeholder="Поділіться новиною для ваших груп..."
          ...
          {groupActivityPosts.map((post) => (
            <article key={post.id} className="rounded-[6px] border border-[rgba(255,255,255,0.1)] bg-nebori-panel p-2.5">
              ...
              <Link href={`/groups/activity/${post.id}`} className="relative mt-3 block h-[220px] overflow-hidden rounded-[4px] border border-[rgba(255,255,255,0.1)] bg-black lg:h-[250px]">
                <img
