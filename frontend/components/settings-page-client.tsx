      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-nebori-muted">
            @nebori_user • параметри сайту та акаунта
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <section className="space-y-2 rounded-[4px] border border-[rgba(255,255,255,0.08)] p-3">
          ...
          <Toggle
            ...
            hint="Автоматично запускати наступне відео."
          />
          <Toggle
            checked={subtitles}
            onChange={setSubtitles}
            label="Субтитри за замовчуванням"
            hint="Вмикати субтитри, якщо вони доступні."
          />
          <label className="block rounded-[4px] border border-[rgba(255,255,255,0.1)] bg-nebori-panel px-3 py-2">
            <p className="text-sm font-semibold text-[#e6e9f3]">
              Якість відео за замовчуванням
            </p>
            <p className="mb-2 text-xs text-nebori-muted">
              Оберіть пріоритет при старті відтворення.
            </p>
        ...
        <section className="space-y-2 rounded-[4px] border border-[rgba(255,255,255,0.08)] p-3">
          ...
          <Toggle
            ...
            label="Коментарі"
            hint="Сповіщати про відповіді та згадки."
          />
          <Toggle
            checked={notifySubscriptions}
            onChange={setNotifySubscriptions}
            label="Підписки"
            hint="Нові відео від каналів, на які ви підписані."
          />
          <Toggle
            checked={emailDigest}
            onChange={setEmailDigest}
            label="Email-дайджест"
            hint="Щотижневе зведення активності на пошту."
          />
        </section>

        <section className="space-y-2 rounded-[4px] border border-[rgba(255,255,255,0.08)] p-3">
          <h2 className="text-lg font-semibold text-[#e6e9f3]">
            Приватність та інтерфейс
          </h2>
          <Toggle
            checked={historyEnabled}
            onChange={setHistoryEnabled}
            label="Зберігати історію переглядів"
            hint="Використовується для рекомендацій та історії."
          />
          <Toggle
            checked={showOnline}
            onChange={setShowOnline}
            label="Показувати статус онлайн"
            hint="Інші користувачі бачитимуть, що ви в мережі."
          />
          <Toggle
            checked={compactCards}
            onChange={setCompactCards}
            label="Компактні картки відео"
            hint="Менші превʼю в стрічці для щільнішого списку."
          />
          <button
            type="button"
